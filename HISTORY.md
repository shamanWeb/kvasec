...
- С текущего релиза для удаления пакета KVASEC будет использоваться только команда uninstall, чтобы не производить путаницы команда remove - удалена.
...
- При установке пакета, теперь есть возможность сразу выбрать и подключить одну домашнюю сеть к KVASEC.
...
- Доработан процесс удаления пакета, внесен новый ключ при удалении "develop", при использовании которого останавливаются и удаляются все связанные с KVASEC сервисы и их файлы конфигурации (Благодарность [@dimbady](https://t.me/dimbady)).
...
- Отключен вывод в лог роутера сообщения "Сброс пакета KVASEC в исходное состояние ЗАВЕРШЕН!", а так же, проверен на ошибки код в файле /opt/etc/ndm/netfilter.d/100-vpn-mark [issue [#104](https://github.com/qzeleza/kvas/issues/104)].
...
## 1.1.7
- Добавлена поддержка доменных имен в файле **/opt/etc/hosts**, что позволяет быстро добавлять собственные доменные имена для локальной сети и не только; При этом, данный список атоматически добавяется в конфигурацию одного из DNS серверов, установленных KVASEC: dnsmasq или AdGuardHome.
...
- Реализована возможность добавлять клиентов **SSTP** сетей для подключения к **KVASEC** командой `vpn net add`.
- Реализовано автоматическое обновление списка VPN интерфейсов, при их создании или удалении (кроме IPSpec)
- Реализован механизм очистки правил **iptables** для интерфейсов **PPTP** и **L2TP**, через которые происходит подключение к провайдеру, с целью решить вопрос возобновление работы **KVASEC** при обрыве соединения такого рода [#issue](https://github.com/qzeleza/kvas/issues/97).
...
- Появилась возможность добавлять из буфера обмена сразу ссылку на страницу, KVASEC позаботится и оставит от адреса только доменное имя: https://dom1.dom2.domain.com/art/.
...
- Изменен адрес проверки работы KVASEC на клиентах на более быстрый вариант: с 2ip.ru на myip2.ru
...
- Теперь нет необходимости ставить * перед доменым именем. При добавлении KVASEC сделает запрос - добавлять ли поддомены или оставить как есть.
...
- Изменен принцип проверки работы службы AdGuardHome - ориентация на наличие запущенной службы и соответствующей записи в файле настроек KVASEC
...
- реализованы новые команды
     ...
     - 'adguard on' - подключает использование AdGuard Home к KVASEC
     - 'adguard off' - отключает использование AdGuard Home в KVASEC
...
- внесены правки в код для проверки работоспособности пакета AdGuard Home в связке с KVASEC.
