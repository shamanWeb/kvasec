...
Отдельно: **Telegram** использует хардкод-IP дата-центров (91.108.x / 149.154.x), а НЕ DNS →
`kvas add` по домену его не завернёт в тоннель. Чтобы убрать нужду в MTProxy, надо добавить
эти CIDR прямо в KVAS_LIST с персистентностью (пока не сделано; MTProxy — рабочий обход).
