...
Собирается **core KVASEC**: AmneziaWG-маршрутизация + adblock + селективный роутинг.
...
./build.sh 1.3.0      # или явным аргументом
...
- **автоматически:** первый merge после миграции публикует `1.3.0`, затем каждый merge/push
  в `master` увеличивает PATCH (`1.3.1`, `1.3.2`, …);
...
Это основной путь публикации. После `1.3.0` CI увеличивает PATCH при каждом merge.
...
`1.1.9_beta-10-N` корректно распознаются как `1.1.9` и обновляются до `1.3.0`.
