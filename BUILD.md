...
Это основной путь публикации. Поднимать `VERSION` для каждого merge не обязательно:
CI возьмёт `max(VERSION, предыдущий release + 1)`. Но перед заметным релизом полезно
обновлять `VERSION`, чтобы локальная сборка и метаданные репозитория оставались синхронны.
...
Дальше CI вычисляет новый номер → запускает `./build.sh <N>` → публикует Release
`v1.1.9_beta-10-<N>` как `latest`. `kvas upgrade` видит его через `/releases/latest`.
...
Нюансы:
- **Тег `vN` — альтернативный ручной триггер;** сам релиз выходит под именем
  `v1.1.9_beta-10-N`. Не используйте его для обычного merge в `master`.
- **Номер должен расти** — `upgrade` (фикс v43) не даунгрейдит: если в релизе `≤` установленного, скажет «Квас все еще свеж».
- **VERSION** — минимум для CI и номер по умолчанию для локального `./build.sh`; merge-CI
  может выпустить номер больше, если прошлый release уже новее этого файла.
...
## 4. Версионирование
`build.sh` берёт номер из `VERSION` либо аргумента. Для merge в `master` CI использует
`max(VERSION, предыдущий release + 1)`, поэтому обновление всегда имеет новый номер.
Для ручного workflow и тегов номер задаётся явно. `kvas upgrade` сравнивает
`APP_VERSION`/`APP_RELEASE` и не ставит пакет с номером, меньшим либо равным установленному.
